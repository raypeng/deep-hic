...
def generate_pairs(seq, interactions, seq_length, start_pos, end_pos):
    ...
    b3 = np.zeros((max_num_pairs, seq_length, 4))
    ...
    return b1[:num_pairs], b2[:num_pairs], b3[:num_pairs], dist[:num_pairs], val[:num_pairs], indices
...
if __name__ == '__main__':
    ...
    b1, b2, b3, dist, val, indices = generate_pairs(seq, interactions, length, start_pos, end_pos)
    print len(indices), 'pairs in total'

    cache_file = 'chr{0}_{1}_kr_mat-mid_{2}_{3}.h5'.format(chromosome, res_str, start_pos, end_pos)
