...
data_path = 'data/chr{0}_{1}k_kr_mat_{2}_{3}.h5'.format(chromosome, res / 1000, start_pos, end_pos)
...
model_chr = int(model_file.rsplit('/', 1)[-1][3:5])
print 'model_chr', model_chr
if model_chr == chromosome:
...
else: # predicting on another chromosome
...
np.save('matrices/chr{0}_{1}k_{2}_{3}_{4}_pred_mat'.format(chromosome, res / 1000, mode_str, start_pos, end_pos), pred_mat)
np.save('matrices/chr{0}_{1}k_{2}_{3}_{4}_true_mat'.format(chromosome, res / 1000, mode_str, start_pos, end_pos), true_mat)
print 'mat saved'
